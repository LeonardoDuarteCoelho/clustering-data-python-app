...
st.title("Análise de Agrupamento de Interesses de Usuários")
st.write("""
...
up_file = st.file_uploader("Faça o upload do arquivo CSV para realizar a previsão", type=["csv"])


def process_predict(df):
    enconded_sex = encoder.transform(df[['sexo']])
    encoded_df = pd.DataFrame(enconded_sex, columns=encoder.get_feature_names_out(['sexo']))
    data = pd.concat([df.drop(columns=['sexo'], axis=1), encoded_df], axis=1)

    escalated_data = scaler.transform(data)
    cluster = kmeans.predict(escalated_data)
    return cluster
...
if up_file is not None:
    ...
    st.download_button(label='Baixar resultados completos', data=csv, file_name='grupos_interesses.csv', mime='text/csv')
